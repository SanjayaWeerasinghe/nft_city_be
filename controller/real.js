...
registerNft = async (req, res) => {
    ...
    try {
        ...
        const blockchainTokenId = lastNft ? Number(lastNft.blockchain_token_id) + 1 : 1;
        ...
        // Mint NFT on blockchain - FIXED: Properly handle unsubscribe
        // Mint NFT on blockchain - Alternative approach
        ...
        console.log("Blockchain transaction result:", blockHash);
        ...
        console.log("registerNft error", e.message);
        ...
        // Optional: Clean up uploaded file if minting fails
        // if (e.message.includes('minting failed') && md5) {
        //     try {
        //         await deleteUploadedFile(`images/${md5}.${extension}`);
        //     } catch (deleteError) {
...
mintNft = async (req, res) => {
    try {
        ...
        successResponse(res, true, "NFT minted successfully", 200, { blockHash: "mintedNft hash" })
...
addOrder = async (req, res) => {
    try {
        ...
        //     const price = BigInt(tokenDetail.price) * BigInt(1e18);
        //     const royaltyPercent = BigInt(tokenDetail.royalties) * BigInt(100);

        //     await global.api.tx.nft.addOrder(body.token_id, price, body.order_number, royaltyPercent, newPair.address)
        ...
        // if (blockHash.status === 'ExtrinsicSuccess') {
        let orderhash = Math.floor(Math.random() * 100)
